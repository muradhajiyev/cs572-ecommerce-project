    ...
    .then(() => {
      res.status(200).json(
        new ApiResponse(200, "success", {
          success: "address saved to a particular buyer",
        })
      );
    })
    .catch((err) => {
      res
        .status(500)
        .send(new ApiResponse(500, "error", { message: err.message }));
    });
};
  ...
  Buyer.findById(req.user._id)
    ...
    .then(() => {
      res
        .status(200)
        .json(
          new ApiResponse(200, "success", { success: "follow seller deleted" })
        );
    })
    .catch((err) => {
      res.status(500).send(new ApiResponse(500, "error", err));
...
exports.deleteReview = (req, res, next) => {
  Product.findById(req.params.productId)
    ...
    .then(() => {
      res
        .status(200)
        .json(new ApiResponse(200, "success", { success: "review deleted" }));
    })
    .catch((err) => {
      res.status(500).send(new ApiResponse(500, "error", err));
    });
